...
export default function Home() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (Array.isArray(data.text)) setServerText(data.text);
    ...
    return (
        <main className="min-h-screen bg-gradient-to-b from-gray-50 to-white text-gray-900">
            <div className="mx-auto max-w-2xl px-4 py-10">
                <h1 className="text-3xl font-semibold tracking-tight">VectorAI Assistant</h1>
                <p className="mt-2 text-gray-600">Upload an image or PDF. We’ll analyze it and extract insights.</p>
                ...
                <form onSubmit={handleSubmit} className="mt-8 space-y-6">
                    ...
                    {/* ----- RESULTS SECTION ----- */}
                    {(serverUrl || serverLabels.length > 0 || serverText.length > 0) && (
                        <div className="space-y-4 border-t pt-6">
                            <h3 className="text-lg font-medium text-gray-900">Analysis Results</h3>

                            {serverUrl && (
                                <div className="text-sm">
                                    <a href={serverUrl} target="_blank" rel="noreferrer" className="text-blue-600 underline hover:text-blue-800">
                                        View Uploaded File
                                    </a>
                                </div>
                            )}
                            ...
                            <div className="grid gap-4 md:grid-cols-2">
                                ...
                                {serverLabels.length > 0 && (
                                    <div className="rounded border border-gray-100 bg-white p-4 shadow-sm">
                                        <strong className="block mb-2 text-sm font-semibold text-gray-700">Detected Objects (Labels):</strong>
                                        <ul className="space-y-1 text-sm text-gray-600">
                                            {serverLabels.map((l, i) => (
                                                <li key={i} className="flex justify-between">
                                                    <span>{l.Name}</span>
                                                    <span className="text-gray-400">{l.Confidence ? `${l.Confidence.toFixed(0)}%` : ""}</span>
                                                </li>
                                            ))}
                                        </ul>
                                    </div>
                                ...
                                {serverText.length > 0 && (
                                    <div className="rounded border border-gray-100 bg-white p-4 shadow-sm">
                                        <strong className="block mb-2 text-sm font-semibold text-gray-700">Detected Text (OCR):</strong>
                                        <div className="max-h-60 overflow-y-auto rounded bg-gray-50 p-2 text-sm text-gray-700">
                                            {serverText.map((line, i) => (
                                                <p key={i} className="mb-1 border-b border-gray-100 pb-1 last:border-0 last:pb-0">
                                                    {line}
                                                </p>
                                            ))}
                                        </div>
